...
class TournamentAdmin(admin.ModelAdmin):
    list_display = [
        "title", "classroom", "type_badge",
        "status_badge", "participants_count",
        "start_time", "end_time",
    ]
...
class TournamentParticipantAdmin(admin.ModelAdmin):
    list_display = [
        "rank_display", "student", "tournament",
        "score_display", "registered_at",
    ]
    ...
    def rank_display(self, obj):
        ...
        if rank == 1:
            ...
            return format_html('<span style="font-size:15px;">🥈 2</span>')
        elif rank == 3:
            return format_html('<span style="font-size:15px;">🥉 3</span>')
        return format_html('<span style="color:#6b7280;font-weight:600;">#{}</span>', rank)

    @admin.display(description="Ball")
    def score_display(self, obj):
        return format_html(
            '<span style="color:#6366f1;font-weight:700;">⭐ {}</span>', obj.score
        )
...
class ChallengeAdmin(admin.ModelAdmin):
    list_display = [
        "challenger", "vs_display", "quiz",
        "status_badge", "xp_stake_display", "expires_at",
    ]
    list_filter = ["status"]
    ...
    def xp_stake_display(self, obj):
        return format_html(
            '<span style="color:#f59e0b;font-weight:700;">⭐ {} XP</span>', obj.xp_stake
        )
